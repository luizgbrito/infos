var campaignID = "678bb1ec71994791219978c5";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("lp.potencialenlinea.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://lp.potencialenlinea.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://lp.potencialenlinea.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("lp.potencialenlinea.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
